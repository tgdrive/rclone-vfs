...
type sequential struct {
	...
	o                vfscommon.RemoteObject // source to read from
...
func newSequential(ctx context.Context, o vfscommon.RemoteObject, initialChunkSize int64, maxChunkSize int64) ChunkedReader {
	return &sequential{
		ctx:              ctx,
		o:                o,
		offset:           -1,
...
func (cr *sequential) openRange() error {
	...
	if length <= 0 {
		if offset == 0 {
			...
			rc, err = cr.o.Open(cr.ctx, &vfscommon.RangeOption{Start: offset, End: -1})
		}
	} else {
		rc, err = cr.o.Open(cr.ctx, &vfscommon.RangeOption{Start: offset, End: offset + length - 1})
	}
	if err != nil {
		return err
	}
