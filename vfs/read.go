...
func newReadFileHandle(f *File) (*ReadFileHandle, error) {
	...
	if f.d.vfs.cache != nil {
		cachePath := f.Path()
		item := f.d.vfs.cache.Item(cachePath)
		if item == nil {
			return nil, fmt.Errorf("failed to get cache item for %s", cachePath)
		}

		// If there's no remote object set, try to create one from the VFS
		if f.remote != nil {
			err := item.Open(f.remote)
			if err != nil {
				return nil, fmt.Errorf("vfs read: failed to open cache item: %w", err)
			}
...
func (fh *ReadFileHandle) readAt(p []byte, off int64) (n int, err error) {
	if fh.f.d.vfs.cache != nil {
		item := fh.f.d.vfs.cache.Item(fh.f.Path())
		if item != nil {
