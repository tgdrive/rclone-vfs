...
type Dir struct {
	...
	modTime time.Time
	children map[string]Node // direct children known to this VFS
}

// newDir creates a new directory
func newDir(vfs *VFS, parent *Dir, name string) *Dir {
...
func (d *Dir) Stat(filePath string) (node Node, err error) {
	...
	if d.vfs.cache != nil {
		cachePath := d.vfs.cache.Item(filePath)
		if cachePath != nil && cachePath.Exists() {
			f := newFile(d.vfs.Context(), d, filePath)
