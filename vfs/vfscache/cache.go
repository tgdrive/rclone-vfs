...
// context passed in.
func New(ctx context.Context, opt *vfscommon.Options, avFn AddVirtualFn) (*Cache, error) {
	if opt.CacheDir == "" {
		return nil, errors.New("vfs cache: CacheDir not set")
	}
	root := filepath.Join(opt.CacheDir, "vfs")
	metaRoot := root + "-meta"

	// Create directories
	for _, d := range []string{root, metaRoot} {
		if err := os.MkdirAll(d, 0700); err != nil {
			return nil, fmt.Errorf("vfs cache: failed to create cache directory %q: %w", d, err)
		}
	}
...
func (c *Cache) Rename(name string, newName string, newObj vfscommon.RemoteObject) (err error) {
	item, _ := c.get(name)
