...
import (
	...
	"github.com/tgdrive/rclone-vfs/vfs/vfscache/downloaders"
...
type Item struct {
	...
	o               vfscommon.RemoteObject   // object we are caching - may be nil
...
type StoreFn func(vfscommon.RemoteObject)
...
func (item *Item) Open(o vfscommon.RemoteObject) (err error) {
	for range 3 {
		item.preAccess()
		err = item.open(o)
		item.postAccess()
...
//
// call with lock held
func (item *Item) _checkObject(o vfscommon.RemoteObject) error {
	if o == nil {
...
func (item *Item) _ensure(offset, size int64) (err error) {
	...
	if item.downloaders == nil {
		...
		if item.o == nil {
			return errors.New("vfs cache: no remote object available for download")
		}
		item.downloaders = downloaders.New(item.c.ctx, item, item.c.opt, item.name, item.o)
	}
...
func _fingerprint(o vfscommon.RemoteObject, fast bool) string {
...
// rename the item
func (item *Item) rename(name string, newName string, newObj vfscommon.RemoteObject) (err error) {
	item.preAccess()
	defer item.postAccess()
	item.mu.Lock()
