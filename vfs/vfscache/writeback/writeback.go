...
func (wb *WriteBack) _remove(id Handle) (found bool) {
	...
	if found {
		fmt.Fprintf(os.Stderr, "vfs cache: cancelling writeback (uploading %v) %p item %d\n", wbItem.uploading, wbItem, wbItem.id)
		if wbItem.uploading {
			// We are uploading already so cancel the upload
			wb._cancelUpload(wbItem)
		}
...
func (wb *WriteBack) upload(ctx context.Context, wbItem *writeBackItem) {
	...
	fmt.Fprintf(os.Stderr, "vfs cache: starting upload\n")
...
func (wb *WriteBack) _cancelUpload(wbItem *writeBackItem) {
	...
	fmt.Fprintf(os.Stderr, "vfs cache: cancelling upload\n")
	...
	wb._pushItem(wbItem)
	fmt.Fprintf(os.Stderr, "vfs cache: cancelled upload\n")
}
...
func (wb *WriteBack) processItems(ctx context.Context) {
	...
	for wbItem := wb._peekItem(); wbItem != nil && time.Until(wbItem.expiry) <= 0; wbItem = wb._peekItem() {
		...
		if wb.uploads >= 4 {
			fmt.Fprintf(os.Stderr, "vfs cache: delaying writeback as max transfers exceeded\n")
			resetTimer = false
			break
		}
