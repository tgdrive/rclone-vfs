...
type Downloaders struct {
	...
	src    vfscommon.RemoteObject // source object
	remote string
	wg     sync.WaitGroup
...
type downloader struct {
	...
	in        io.ReadCloser // input we are reading from
...
func New(ctx context.Context, item Item, opt *vfscommon.Options, remote string, src vfscommon.RemoteObject) (dls *Downloaders) {
	if src == nil {
	...
	dls.wg.Go(func() {
		...
		for {
			...
			case <-ticker.C:
				err := dls.kickWaiters()
				if err != nil {
					fmt.Fprintf(os.Stderr, "vfs cache: failed to kick waiters: %v\n", err)
				}
			case <-ctx.Done():
				return
			}
...
func (dls *Downloaders) _countErrors(n int64, err error) {
	if err == nil && n != 0 {
		if dls.errorCount != 0 {
			fmt.Fprintf(os.Stderr, "vfs cache: downloader: resetting error count to 0\n")
			dls.errorCount = 0
			dls.lastErr = nil
		}
		return
	...
	if err != nil {
		...
		//}
		dls.lastErr = err
		fmt.Fprintf(os.Stderr, "vfs cache: downloader: error count now %d: %v\n", dls.errorCount, err)
	}
}
...
func (dls *Downloaders) _newDownloader(r ranges.Range) (dl *downloader, err error) {
	...
	dl.wg.Go(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "vfs cache: failed to download: %v\n", err)
		}
...
func (dls *Downloaders) kickWaiters() (err error) {
	...
	for _, waiter := range dls.waiters {
		...
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "vfs cache: restart download failed: %v\n", err)
		}
	}
	if dls.lastErr != nil && strings.Contains(dls.lastErr.Error(), "no space") {
		fmt.Fprintf(os.Stderr, "vfs cache: cache is out of space %d/%d: last error: %v\n", dls.errorCount, maxErrorCount, dls.lastErr)
		dls._closeWaiters(dls.lastErr)
		return dls.lastErr
	}

	if dls.errorCount > maxErrorCount {
		fmt.Fprintf(os.Stderr, "vfs cache: too many errors %d/%d: last error: %v\n", dls.errorCount, maxErrorCount, dls.lastErr)
		dls._closeWaiters(dls.lastErr)
		return dls.lastErr
	}
...
func (dl *downloader) Write(p []byte) (n int, err error) {
	...
	defer func() {
		...
		if waitErr := dl.dls.kickWaiters(); waitErr != nil {
			fmt.Fprintf(os.Stderr, "vfs cache: download write: failed to kick waiters: %v\n", waitErr)
			if err == nil {
				err = waitErr
			}
		}
...
loop:
	...
	// If the downloader was stopped, signal end of stream
	if dl.stop && err == nil {
		err = io.EOF
	}
	return n, err
...
func (dl *downloader) _stop() {
	...
	// stop the downloader by closing the reader
	if dl.in != nil {
		dl.in.Close()
	}
}
